...
fn gen_simple_inc(
    ...
    let end_stage_1 = &step_stage & (&addr_step).equal(&addr_step_max);
    ...
    mach_out_1.memr = end_stage_1.clone();
    mach_out_1.memw = BoolVarSys::from(false);
    // 2. Increment and write memory
    let mut mach_out_2 = InfParOutputSys::new(config);
    mach_out_2.state = to_mach_state(2u32.into(), addr_step_zero.clone(), false.into());
    mach_out_2.memr = BoolVarSys::from(false);
    mach_out_2.memw = BoolVarSys::from(true);
    mach_out_2.memval = &mobj.in_memval + 1u32;
    // 3. Move back positions
    let mut mach_out_3 = InfParOutputSys::new(config);
    mach_out_3.state = to_mach_state(2u32.into(), addr_step_zero.clone(), !&step_stage);
    mach_out_3.dkind = int_ite(
        step_stage.clone(),
        U2VarSys::from(DKIND_MEM_ADDRESS),
        U2VarSys::from(DKIND_PROC_ID),
    );
    mach_out_3.dpmove = DPMOVE_BACKWARD.into();
    mach_out_3.stop = &step_stage & !&mobj.in_dp_move_done;
    let mut mach_out_empty = InfParOutputSys::new(config);
    mach_out_empty.state = to_mach_state(0u32.into(), addr_step_zero.clone(), false.into());
    // join
    let final_state = dynint_table(
        UDynVarSys::from(main_stage),
        [mach_out_1, mach_out_2, mach_out_3, mach_out_empty].map(|v| v.to_dynintvar()),
    );
